...
bool UARToolkitComponent::LoadMarkers(TArray<FString> markers){
	return IARToolkitPlugin::GetARToolkitDeviceSafe()->LoadMarkers(markers, OnMarkerLoaded);
}

bool UARToolkitComponent::LoadMarkersNFT(TArray<FString> markers){
	
		return IARToolkitPlugin::GetARToolkitDeviceSafe()->LoadMarkersNFT(markers, OnMarkerLoaded, OnNFTReady);
