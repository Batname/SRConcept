...
//ANDROID
	...
	DataPath = FPaths::ProjectContentDir() + "ARToolkit/";
	DataPath = FPaths::ConvertRelativePathToFull(DataPath); //Absolute path
#endif

															//IPHONE
#if defined IPHONE_ONLY
	DataPath = ConvertToIOSPath(FString::Printf(TEXT("%s"), FApp::GetProjectName()).ToLower() + FString("/content/artoolkit/"), 0);
	DataPath = FPaths::ConvertRelativePathToFull(DataPath); //Absolute path
	DataPath = DataPath.Replace(L"/Documents/", L"/Library/Caches/");
...
bool FARToolkitDevice::LoadMarkers(TArray<FString> markerNames, FMarkerLoadedDelegate OnMarkerLoaded){
	...
	for (int32 i = 0; i != markerNames.Num(); ++i){
		...
		//Broadcast
		OnMarkerLoaded.Broadcast(markerName, i + 1, markerNames.Num(),"FIDUCIAL");
	}
	return 1;
}

bool FARToolkitDevice::LoadMarkersNFT(TArray<FString> markerNames, FMarkerLoadedDelegate OnMarkerLoaded, FNFTCompleteDelegate OnNFTReady){
	if (!initiated) return false;
	
	

	Future = Async<bool>(EAsyncExecution::ThreadPool, [=]
	{
		...
		for (int32 i = 0; i != markerNames.Num(); i++){
			...
			//Broadcast
			OnMarkerLoaded.Broadcast(markerName, i+1, markerNames.Num(),"NFT");
		...
		//Notify if NFT init is completed
		OnNFTReady.Broadcast();
...
{
	while (Future.Get() == (uintptr_t)nullptr) {
		//Just wait to finsih NFT init
	}
